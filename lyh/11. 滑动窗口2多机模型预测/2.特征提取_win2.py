...
if __name__ == "__main__":
    ...
    # 测试数据
    saveFaultyCoreDict(os.path.join(tmpsavepath2, "测试数据_正常_异常_win2"), faulty_core_pd_dict)
    ...
    saveFaultyDict(os.path.join(tmpsavepath2, "数据修订版正常1km_win2"), normal_1km_pd)
    ...
    saveFaultyDict(os.path.join(tmpsavepath2, "数据修订版正常3km_win2"), normal_3km_pd)
    ...
    saveFaultyDict(os.path.join(tmpsavepath2, "数据修订版正常9km_win2"), normal_9km_pd)
    ...
    if err:
        print("1KM合并失败")
        exit(1)
    merge_normal_1km.to_csv(os.path.join(tmpsavepath2, "数据修订版正常1KM_win2_核数据合并.csv"))

    merge_normal_3km, err = mergeDataFrames(list(normal_3km_pd.values()))
    if err:
        print("3KM合并失败")
        exit(1)
    merge_normal_3km.to_csv(os.path.join(tmpsavepath2, "数据修订版正常3KM_win2_核数据合并.csv"))
    ...
    if err:
        print("9KM合并失败")
        exit(1)
    merge_normal_9km.to_csv(os.path.join(tmpsavepath2, "数据修订版正常9KM_win2_核数据合并.csv"))

    # 将测试数据的核心数据合并
    faulty_dict = merge_int_int_DataFrame(faulty_core_pd_dict)
    saveFaultyDict(os.path.join(tmpsavepath2, "测试数据正常_异常_核数据合并_win2"), faulty_dict)
