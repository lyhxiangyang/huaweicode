...
if __name__ == "__main__":
    ...
    #读取1km中的所有异常数据 包含正常
    singleexcludeFaulty = [41, 42, 43, 44, 45, 71, 72, 73, 74, 75]
    faulty_core_pd_dict_1km, err = readFaultyPD(datasavepath, readDir="数据修订版_多机_1KM_process1", excludeFaulty=singleexcludeFaulty, select_feature=process_features)
    # 保存测试数据
    saveFaultyCoreDict(os.path.join(tmpsavepath1, "数据修订版_多机_1KM_所有数据"), faulty_core_pd_dict_1km)


    #读取3km中的所有异常数据 包含正常
    singleexcludeFaulty = [41, 42, 43, 44, 45, 71, 72, 73, 74, 75]
    faulty_core_pd_dict_3km, err = readFaultyPD(datasavepath, readDir="数据修订版_多机_3KM_process1", excludeFaulty=singleexcludeFaulty, select_feature=process_features)
    # 保存测试数据
    saveFaultyCoreDict(os.path.join(tmpsavepath1, "数据修订版_多机_3KM_所有数据"), faulty_core_pd_dict_3km)


    #读取9km中的所有异常数据 包含正常
    singleexcludeFaulty = [41, 42, 43, 44, 45, 71, 72, 73, 74, 75]
    faulty_core_pd_dict_9km, err = readFaultyPD(datasavepath, readDir="数据修订版_多机_9KM_process1", excludeFaulty=singleexcludeFaulty, select_feature=process_features)
    # 保存测试数据
    saveFaultyCoreDict(os.path.join(tmpsavepath1, "数据修订版_多机_9KM_所有数据"), faulty_core_pd_dict_3km)
