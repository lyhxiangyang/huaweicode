...
if __name__ == "__main__":
    ...
    for itype in MODEL_TYPE:
        ...
        for i in tallFault:
            if i not in tDic.keys():
                tDic[i] = {}
            tmetrics = get_metrics(getTestRealLabels(), getTestPreLabels(), i)
            # 将数据进行输出并保存
    ...
    print(itpd)
    itpd.to_csv(os.path.join(saverespath, "多机5_预测数据统计信息.csv"))
    print("=========================")
    print("输出信息->", os.path.join(saverespath, "多机5_预测数据统计信息.csv"))
    print("模型预测结束")
