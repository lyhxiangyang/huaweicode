...
trainDataPath = [
    "tmp/tData/多机-E5-process-server-1KM/所有process错误码信息/0.csv",
    "tmp/tData/多机-E5-process-server-1KM/所有process错误码信息/15.csv",
    "tmp/tData/多机-E5-process-server-1KM/所有process错误码信息/65.csv",
    "tmp/tData/多机-红区-process-server-1KM/所有process错误码信息/0.csv",
]
testDataPath = [
    "tmp/tData/多机-红区-process-server-1KM/所有process错误码信息/0.csv",
    "tmp/tData/多机-红区-process-server-1KM/所有process错误码信息/15.csv",
    "tmp/tData/多机-红区-process-server-1KM/所有process错误码信息/65.csv",
]
...
def TrainThree(trainedpd : pd.DataFrame, spath:str, modelpath : str = "Classifiers/saved_model/tmp"):
    ...
    for itype in MODEL_TYPE:
        accuracy = model_train(trainedpd, itype, saved_model_path=modelpath)
...
中间文件都放在spath中
"""
def ModelTrainAndTest(trainedpd : pd.DataFrame, testpd : pd.DataFrame, spath:str, modelpath : str = "Classifiers/saved_model/tmp", trainAgain: bool = True):
    # 先生成模型 得到生成模型的准确率
    if not os.path.exists(spath):
        os.makedirs(spath)
    if not os.path.exists(modelpath):
        os.makedirs(modelpath)
    if trainAgain:
        TrainThree(trainedpd, spath, modelpath)
